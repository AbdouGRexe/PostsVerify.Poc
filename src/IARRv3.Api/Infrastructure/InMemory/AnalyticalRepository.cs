...
public class AnalyticalRepository : IAnalyticalRepository
{
    ...
    public Task<ReadOnlyDictionary<Guid, UserAggregation>> GetUserAggregations(IEnumerable<Guid> userIds)
    {
        try
        {
            ...
            return Task.FromResult(new ReadOnlyDictionary<Guid, UserAggregation>(aggregations));
        }
        catch (Exception exception)
        {
            return Task.FromException<ReadOnlyDictionary<Guid, UserAggregation>>(exception);
        }
    }
}