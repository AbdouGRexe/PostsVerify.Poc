...
internal class HandleNewRatingsIARRSevice(
    double stopValue,
    IReadOnlyDictionary<int, UserAggregation> userAggregations,
    Network network,
    UserData cachedData)
    : IHandleNewRatingsService
{
    ...
    private List<Post> GetUpdatedPostsWithEstimatedAccuracy()
    {
        ...
        foreach (var post in _extendedNetwork.Posts)
        {
            ...
            post.EstimatedAccuracyScore = summedReps == 0 ? post.EstimatedAccuracyScore : maxRep * (summedWeighedRatings / summedReps);
    ...
    private void UpdateUsersReputation(List<Rating> aggregatedRatings)
    {
        foreach (var user in _extendedNetwork.Users)
        {
            ...
            var maxNbrRatings = _extendedNetwork // in this case we only have _extended network otherwise we'd use aggregation 
                .Ratings
